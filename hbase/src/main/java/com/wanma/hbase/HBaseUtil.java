...
//		cfg.set("hbase.zookeeper.quorum", hostName);
        ...
        cfg.setInt("hbase.client.scanner.timeout.period",20000);
...
//			}
    ...
    public List<Map<String, String>> selectByRowKey(String tablename, List<Filter> arr, List<String> columns, String startKey, String endKey)
    {
        ...
        try {
            ...
            Scan s1 = new Scan();
            s1.setCaching(5000);
            s1.setStartRow(Bytes.toBytes(startKey));
            s1.setStopRow(Bytes.toBytes(endKey));
            for (Filter v : arr) {
                filterList.addFilter(v);
            }
            if (filterList.filterRow()) {
            ...
            if ((columns != null) && (columns.size() > 0)) {
                for (String s : columns) {
                    s1.addColumn(Bytes.toBytes("sx"), Bytes.toBytes(s));
                }
            }
            ResultScanner ResultScannerFilterList = table.getScanner(s1);
            Map<String, String> map = null;
    ...
    public Map<String, Object> selectByPageFilter(String tablename, List<Filter> conditions, List<String> columns, long minStamp, long maxStamp, int startNum, int pageSize)
            throws Throwable
    {
        Map pageMap = new HashMap();
        Map[] resultList = new Map[10];
        try {
            HTable table = new HTable(cfg, tablename);
            ...
            int i = 0;
            int countAll = 0;
            for (Result rr : resultScannerFilterList) {
                countAll++;
            }
            resultScannerFilterList.close();
            ResultScanner resultScannerFilterList1 = table.getScanner(s1);
            int startIndex = countAll - startNum * pageSize;
            int lastIndex = startIndex + pageSize;
            for (Result rr : resultScannerFilterList1) {
                if ((i++ >= startIndex) && (i <= startIndex + pageSize)) {
                    ...
                    resultList[(lastIndex - i)] = map;
                }
                if (i == startIndex + pageSize)
                    break;
            }
            resultScannerFilterList1.close();
            pageMap.put("pageData", resultList);
            pageMap.put("count", Double.valueOf(Math.ceil(Double.valueOf(i).doubleValue() / pageSize)));
        } catch (IOException e) {
            e.printStackTrace();
        }
        return pageMap;
