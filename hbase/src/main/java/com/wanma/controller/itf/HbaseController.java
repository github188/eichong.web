...
import com.epcentre.server.Global;
...
public class HbaseController {
	...
	public String getRealtimeData(HttpServletRequest request) {
		...
		if (chargeStatus != null) {
			if (chargeStatus == 3) {
				...
				result.put("bpHighestVoltage", make3_33(map.get("3_33")));
    ...
    private String make3_33(Object value) {
        int val = 0;
        if (value != null) {
            val = (new BigDecimal(String.valueOf(value))).multiply(Global.DecTime2).intValue();
        }
        int groupNum = val >> 12;
        int vol = val & 0xFFF;
        return new DecimalFormat("0.00").format(Double.valueOf(vol).doubleValue() / 100.0D) + "V" +
                groupNum + "号组";
    }
	...
	public String getVoltageData(HttpServletRequest request) {
		...
		arr.add(new FilterBuilder(RealtimeConstant.FITER_EQUAL, "sx", "3_1",
				"3").buildFilter());
        String keyHead = epCode + headId + "_";
        Date startDay = new Date(t - 43200000L);
        String startKey = keyHead + sdf.format(startDay);
        Date nowDay = new Date(t);
        String endKey = keyHead + sdf.format(nowDay);
		List<Map<String, String>> chargingList = realtimeUtil.getListByRowKey(
                tableName, arr, null, startKey, endKey);
        ...
        long startTime = new Long((String)startObj.get("ts")).longValue();
        Date startDay1 = new Date(startTime + 1L);
        String startKey1 = keyHead + sdf.format(startDay1);

		// 根据充电开始时间查询充电信息列表
		arr = new ArrayList<Filter>();
		arr.add(new FilterBuilder(RealtimeConstant.FITER_EQUAL, "sx", "epCode",
				epCode).buildFilter());
		List<Map<String, String>> chargingList2 = realtimeUtil.getListByRowKey(
                tableName, arr, null, startKey1, endKey);
        List<Map<String, Object>> cList = new ArrayList<Map<String, Object>>();
		Map<String, Object> tempMap = null;
		Map<String, Object> tempMap2 = new HashMap<String, Object>();
        tempMap2.put("voltageValue", Integer.valueOf(0));
        tempMap2.put("currentValue", Integer.valueOf(0));
        Map<String, String> lastObj = null;
        if ((chargingList2 != null) && (chargingList2.size() > 0))
            lastObj = (Map)chargingList2.get(0);
        else
            lastObj = new HashMap();
        long newStartTime = new Long((String)lastObj.get("ts")).longValue();
        if (newStartTime - startTime > 30000L) {
            tempMap2.put("voltageValue", Integer.valueOf(0));
            tempMap2.put("currentValue", Integer.valueOf(0));
            tempMap2.put("chargingState", Integer.valueOf(3));
            tempMap2.put("allChargeValue", Double.valueOf(0.0D));
            tempMap2.put("presentChargeValue", Double.valueOf(0.0D));
            tempMap2.put("presentChargeFee", Double.valueOf(0.0D));
        } else {
            tempMap2.put("voltageValue", startObj.get("3_3"));
            tempMap2.put("currentValue", startObj.get("3_4"));
            tempMap2.put("chargingState", Integer.valueOf(3));
            tempMap2.put("allChargeValue", startObj.get("4_1"));
            tempMap2.put("presentChargeValue", startObj.get("4_4"));
            tempMap2.put("presentChargeFee", startObj.get("4_2"));
        }
		for (Map<String, String> map : chargingList2) {
            ...
            fillValue(map.get("3_1"), "chargingState", tempMap2, tempMap);
        ...
        if (resultMap != null)
            result.put("chargingStatus", resultMap.get("3_1"));
		return JSON.toJSONString(result);
	}
	...
	public String getOperationData(HttpServletRequest request) {
		...
		try {
            ...
            String startKey = keyHead + monthStr + "01000000";
            String endKey = keyHead + sdf.format(d);
			List<Filter> arr = new ArrayList<Filter>();
			// 根据操作开始时间查询操作列表
			arr = new ArrayList<Filter>();
            ...
            columns.add("ts");
			List<Map<String, String>> chargingList2 = realtimeUtil.getListByRowKey(
                    tableName, arr, columns, startKey, endKey);
			List<Map<String, Object>> cList = new ArrayList<Map<String, Object>>();
			Map<String, Object> tempMap = null;
			for (Map<String, String> map : chargingList2) {
				tempMap = new HashMap<String, Object>();
