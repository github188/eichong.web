...
public class GateServer extends AbstractGameServer{
	...
	public GateServer(){
		...
		if (serverConfigs != null) {
			...
			//gate服务器
			if (serverConfigs.get("data-Analyze-server") !=null) {
				ServerConfig serverConfig = serverConfigs.get("data-Analyze-server");
				
				ByteToMessageDecoder decoder = new AnalyzeNettyDecoder();
				MessageToByteEncoder encoder = new AnalyzeNettyEncoder();
				
				AnalyzeNettyServer nettyServer = new AnalyzeNettyServer(serverConfig, decoder, encoder,0,0);
